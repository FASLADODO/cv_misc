...
if ~exist('matlabVisRoot', 'var')
    run([userpath_fix,filesep,'csc2503_hw',filesep,'matlabVisTools',filesep,'startup.m']);
end
codeDir = [userpath_fix,filesep,'csc2503_hw',filesep,'robust_estimation',filesep,'A2handout'];
...
hmdir = [codeDir,filesep,'cellFinder',filesep];

% Link the other handout directories for A2
addpath([hmdir,'circleFit']);
addpath([hmdir,'util']);
addpath([hmdir,'..',filesep,'data']);
...
sigmaGM = 35; % Geman-McLure estimator scale parameter
...
%                 plotCircle(circles(i,1:2)',circles(i,3),'g-');
%             end
%             plotCircle(circle(1,1:2)',circle(1,3),'r-');
