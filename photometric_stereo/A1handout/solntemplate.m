...
if isunix || ismac
    ...
    if ~exist('matlabVisRoot', 'var')
        run([userpath_fix,'/csc2503_hw/matlabVisTools/startup.m']);
    end
    codeDir = [userpath_fix,'/csc2503_hw/photometric_stereo/A1handout/'];
...
for useImageSet = 3
    % 1:6  or just choose one image set, e.g. 3
    switch useImageSet
        case 1
            name = 'gray';
        case 2
...
imDataColor = zeros(numPixels,nDirChrome);
T = Lchrome'*nCrop';
for j = 1:3
    for i = 1:nDirChrome
        ...
        RGBim = ppmRead(fname);
        imColor = RGBim(:,:,j);
        imDataColor(:,i) = imColor(:);
    end
    imDataColorCrop = imDataColor(mask,:);
    albedo(mask,j) = sum(imDataColorCrop'.*T,1)./sum(T.*T,1);
end
...
[depth] = getDepthFromNormals(reshape(n,[imsize,3]),reshape(maskDepth,imsize));
...
imDataRecCrop = imData(mask,9:12);
Lrec = pinv(g)*imDataRecCrop;
